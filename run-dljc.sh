...
infer_cmd="python3 $DLJC/dljc -t inference --guess --crashExit --checker $CHECKER --solver $SOLVER --solverArgs=\"collectStatistics=true,solver=Z3\" -o logs -m INFER -afud $WORKING_DIR/annotated -- $build_cmd "

# debug_onlyCompile="--onlyCompileBytecodeBase true"
debug_cmd="python3 $DLJC/dljc -t testminimizer --annotationClassPath $ONTOLOGYPATH $debug_onlyCompile --expectOutputRegex 'Z3 Unsatisfiable' --checker $CHECKER --solver $SOLVER --solverArgs=\"collectStatistics=true,solver=Z3\" -o logs -m INFER -afud $WORKING_DIR/annotated -- $build_cmd "
